...
internal static class Calendar {
    internal static readonly ImmutableDictionary<string, string> Applies = new Dictionary<string, string> {
        ...
        { ".cal-row", "relative flex-1 min-h-0 overflow-hidden border-b border-border [transition:opacity_.18s_ease-out]" },
        { ".cal-row.cal-row-fixed", "flex-none [height:var(--row-h,3.05rem)] [min-height:var(--row-h,3.05rem)]" },
        { ".cal-row.cal-row-context", "opacity-55" },
        { ".cal-cell-label", "absolute top-[0.15rem] pl-[0.35rem] text-[0.65rem] text-muted pointer-events-none" },
